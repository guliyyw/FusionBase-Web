  ...
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <div class="w-64 bg-white shadow-md flex flex-col">
      <div class="p-4 border-b">
        <h1 class="text-xl font-bold text-indigo-600">相册管理系统</h1>
      </div>
      ...
      <header class="bg-white shadow-sm">
        <div class="flex justify-between items-center px-6 py-4">
          ...
          <div class="flex items-center space-x-4">
            ...
            <router-link v-if="!isAuthenticated" to="/login" class="text-indigo-600 hover:text-indigo-800">登录</router-link>
            <button v-else @click="logout" class="text-gray-500 hover:text-gray-700 flex items-center">
...
export default {
  setup() {
    ...
    const currentRouteName = computed(() => {
      if (route.path === '/albums') return '我的相册'
      if (route.path === '/shared') return '共享相册'
      if (route.path.startsWith('/album/')) return '相册详情'
      if (route.path === '/album/create') return '创建相册'
      if (route.path.startsWith('/album/edit/')) return '编辑相册'
